...
#Date of last change: Pushkin's birthday 2018 :)
#Version: alpha
#To-do list: 
#1. Open the file with the corpus from Shiny,
#2. Go to sentence (?)
#3. Re-write the same sentence instead of adding a new line

#Instructions for working with languages:
...
#Corpus format: one sentence on one line
#To upload a corpus, use the code as in the example:
#SBC004 <- scan("test/SBC004_fragment.txt", sep = "\n", what= "character", encoding = "UTF-8")

#Very important!!! Note that the corpus should also be in the server() function (see comment with !!!CORPUS!!!)
#and in the list of datasets. For that purpose, check lines 27 (now)
#and line 251.
...
#!!!Add your corpus to this list:
datasets <- c("test_corpus", "SBC004", "ngh_sample", "rus_sample")
...
tabPanel(
  ...
  fluidRow(column(width = 4, selectInput(inputId = "A_Sem", label = h4("Semantic class of A"), 
          ...
          column(width = 4, selectInput(inputId = "A_Ident", label = h4("Identifiability of A"), 
                                choices = list("The referent can be identified by the speaker and the hearer" = "A_Def", "The referent can be identified by the speaker only" = "A_Spec", "The referent can be identified neither by the speaker nor by the hearer" = "A_NonSpec", "Difficult to say" = "A_UndefIdent", "Not applicable (interrogative and impersonal pronouns, clauses, etc.)" = "A_NAIdent"), 
                                selected = 1)
                 ),
...
tabPanel(
    ...
    fluidRow(column(width = 4, selectInput(inputId = "P_Person", label = h4("Person of P"), 
             ...
             column(width = 4, selectInput(inputId = "P_Number", label = h4("Number of P"), 
                                           choices = list("Singular" = "P_SG", "Plural" = "A_PL", "Difficult to say" = "P_undefN", "Clause" = "P_clauseN"), 
                                           selected = 1)
                    )
             ),
    ...
    fluidRow(column(width = 4,selectInput(inputId = "P_Sem", label = h4("Semantic class of P"), 
     ...
     column(width = 4, selectInput(inputId = "P_Ident", label = h4("Identifiability of P"), 
                                           choices = list("The referent can be identified by the speaker and the hearer" = "P_Def", "The referent can be identified by the speaker only" = "P_Spec", "The referent can be identified neither by the speaker nor by the hearer" = "P_NonSpec", "Difficult to say" = "P_UndefIdent", "Not applicable (interrogative and impersonal pronouns, clauses, etc.)" = "A_NAIdent"), 
                                           selected = 1)
            ),
...
switch(input$dataset, "test_corpus" = test_corpus, "SBC004" = SBC004, "ngh_sample" = ngh_sample, "rus_sample" = rus_sample)  #!!!CORPUS!!! Your corpus name should be here in the same format. Add it after a comma.
})
  ...
  if (input$A_radio == 1){
    fluidRow(column(width = 4, textInput(inputId = "A_Head", label = h4("Specify A Head")
          ...
          column(width = 4, selectInput(inputId = "A_Subcat", label = h4("Subcategory of A Head"), 
                                                   choices = list("Proper Noun (persons, institutions, places)" = "A_PropNoun", "Common Noun" = "A_ComNoun", "Personal Pronoun" = "A_PersPro", "Possessive Pronoun" = "A_PossPro", "Demonstrative Pronoun" = "A_DemPro", "Interrogative Pronoun" = "A_InterPro", "Other" = "A_OthPro"), 
                                                   selected = 1
                                                   )
                  )
           )
  ...
  if (input$P_radio == 1){
    fluidRow(column(width = 4, textInput(inputId = "P_Head", label = h4("Specify P Head")
          ...
          column(width = 4, selectInput(inputId = "P_Subcat", label = h4("Subcategory of P Head"), 
                                                   choices = list("Proper Noun (persons, institutions, places)" = "P_PropNoun", "Common Noun" = "P_ComNoun", "Personal Pronoun" = "P_PersPro", "Possessive Pronoun" = "P_PossPro", "Demonstrative Pronoun" = "P_DemPro", "Interrogative Pronoun" = "P_InterPro", "Other" = "P_OthPro"), 
                                                   selected = 1)
                 )
          )
  } 
...
write.table(data.frame(t(c(outmatrixClause, outmatrixA, outmatrixP))), input$inputsLocation, row.names = FALSE, col.names = FALSE, quote = FALSE, append = TRUE)
})
